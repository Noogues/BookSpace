...
function dockClass(isActive: boolean) {
  if (isActive) {
    return 'inline-flex h-11 w-11 items-center justify-center rounded-full bg-teal-500/10 text-teal-700 shadow-sm ring-1 ring-inset ring-teal-500/15 transition-all duration-200 dark:bg-neon-teal/10 dark:text-neon-teal dark:ring-neon-teal/20'
  }
  return 'inline-flex h-11 w-11 items-center justify-center rounded-full text-stone-400 transition-all duration-200 hover:bg-stone-900/5 hover:text-stone-700 dark:text-stone-500 dark:hover:bg-white/5 dark:hover:text-stone-200'
}
...
export function MobileDock() {
  ...
  return (
    <nav
      className="fixed bottom-3 left-1/2 z-40 max-w-[calc(100vw-1rem)] -translate-x-1/2 overflow-x-auto md:hidden"
