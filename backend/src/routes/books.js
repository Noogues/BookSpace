...
const bookInclude = { tags: { include: { tag: true } } };
...
export async function booksRoutes(app, opts) {
    ...
    app.get('/books/:id', async (request, reply) => {
        ...
        if (!book)
            return reply.status(404).send({ error: 'Libro no encontrado' });
        return book;
    });
    ...
    app.post('/books/import', async (request, reply) => {
        ...
        const payload = { imported: created };
        if (errors.length > 0)
